...
public class ResonantCollinearityPt2 {
	...
	private static Set<Vector2Int> antinodeLocatations;
	...
	private static void generateAntinodes(Pair<Integer, Integer> tower1, Pair<Integer, Integer> tower2) {
		...
		antinodeLocatations.add(tower1V);
		antinodeLocatations.add(tower2V);
		
		Vector2Int dist = tower2V.subtract(tower1V);
		Vector2Int antinode1 = tower1V.subtract(dist);
		while(checkBounds(antinode1)) {			
			antinodeLocatations.add(antinode1);
			antinode1 = antinode1.subtract(dist);
		}
		
		Vector2Int antinode2 = tower2V.add(dist);
		while(checkBounds(antinode2)) {			
			antinodeLocatations.add(antinode2);
